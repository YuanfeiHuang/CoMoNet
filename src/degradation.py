...
def stable_anisotropic_gaussian_kernel(sig=2.6,  l=21, scaling=3, angle=0, tensor=False):
    #angle should be range from [0, 1]
    x = sig
    y = np.clip(x*scaling, 0.2, 4.0)
    angle = angle * math.pi
    sig = cal_sigma(x, y, angle)
    k = anisotropic_gaussian_kernel(l, sig, tensor=tensor)
    return k
...
def stable_gaussian_kernel(l=21, sig=2.6, scaling=3, angle=0, aniso=False, tensor=False):
    if aniso:
        return stable_anisotropic_gaussian_kernel(sig=sig, l=l, scaling=scaling, angle=angle, tensor=tensor)
    else:
...
def stable_batch_kernel(batch, l, sig, scaling=3, angle=(0,), aniso=False, tensor=True):
    batch_kernel = np.zeros((batch, l, l))
    for i in range(batch):
        batch_kernel[i] = stable_gaussian_kernel(l, sig[i], scaling, angle[i], aniso, tensor=False)
    return torch.FloatTensor(batch_kernel) if tensor else batch_kernel
...
class BatchSRKernel(object):
    def __init__(self, l, sig, sig_min, sig_max, rate_iso=1.0, scaling=3, angle=(0,), aniso=False):
        self.l = l
        self.sig = sig
    ...
    def __call__(self, random, batch, tensor=False):
        if random == True:  # random kernel
            ...
            return stable_batch_kernel(batch, l=self.l, sig=self.sig, scaling=self.scaling, angle=self.angle, aniso=self.aniso, tensor=tensor)
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, x):
        ...
        if isinstance(self.scale, np.ndarray):
            lr_tensors = self.bicubic(x[0:1], [1 / self.scale[0]])
            lr_tensors = b_GaussianNoising(lr_tensors, Noise_level[0])
            for i in range(B):
                lr_image = self.bicubic(x[i:(i+1)], [1 / self.scale[i]])
                lr_image = b_GaussianNoising(lr_image, Noise_level[i])
                lr_tensors = torch.cat((lr_tensors, lr_image), dim=0)
        else:
            lr_tensors = self.bicubic(x, [1 / self.scale])
            lr_tensors = b_GaussianNoising(lr_tensors, Noise_level.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
